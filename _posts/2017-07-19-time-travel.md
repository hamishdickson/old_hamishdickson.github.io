...
scala> val program: State[Int, (Int, Int, Int)] = for {
     |     a <- get[Int]
     |     _ <- set[Int](a + 1)
     |     b <- get[Int]
     |     _ <- modify[Int](_ + 1)
     ...
     | } yield (a, b, c)
program: cats.data.State[Int,(Int, Int, Int)] = cats.data.StateT@53cb9b5c

scala> val (state, result) = program.run(1).value
state: Int = 3
...
```scala
scala> case class MonadFix[F](f: F) // need to see how this is defined
defined class MonadFix

scala> trait MonadRevState[S, M[_]] {
     |     def get(): MonadFix[S]

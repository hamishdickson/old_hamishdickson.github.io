...
If we are going to do this, why shouldn't we use well grounded mathematics to do it?
...
By the way, since I'm going to be using Scala here, I should mention Scala has special syntax for mutable variables. If it is defined with a `var`, then it's mutable, while a `val` is not. That, as you can probably guess, is a very deliberate decision.
